...
class MetaBoxes
{
    ...
    {
        $product_regular_price = (int)get_post_meta($product->ID, 'product_regular_price', true);
    ...
    public static function save_product_price($product_id)
    {
        ...
        if (isset($_POST['product_regular_price']) && intval($_POST['product_regular_price']) > 0) {
            update_post_meta($product_id, 'product_regular_price', intval($_POST['product_regular_price']));
        }
